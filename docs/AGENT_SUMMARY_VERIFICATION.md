...
2. 或者 summary 太短被过滤了（但已修复）

**代码路径**:
```
analyst_discussion.py (第401行) 
  → 返回 coordinator_summary
  → trading_cycle.py (第384行) 接收 convo
  → trading_cycle.py (第1885行) 返回 "discussion": convo
```

**应该包含**: `discussion.coordinator_summary.summary` 和 `discussion.coordinator_summary.stance`
...
**检查代码**:
- `analyst_discussion.py` 第401行：返回 `coordinator_summary` ✅
- `trading_cycle.py` 第1885行：返回 `"discussion": convo` ✅
- `convo` 应该包含 `coordinator_summary` ✅

**可能原因**:
1. JSON 输出时被截断了（但不太可能）
2. 或者 summary 太短被过滤了（但已修复）

**验证方法**: 
- 运行 `python backend/scripts/check_agent_summaries.py` 检查
- 或者直接检查 `convo.get("coordinator_summary")` 是否存在
...
**修复前**: 
```python
"summary": f"Market stance: {final_stance}. Analysis completed after {len(transcript)} rounds."
```

**修复后**: 
```python
# 从 transcript 中提取更有意义的内容，至少100字符
summary_text = " ".join(summary_parts)  # 合并最后3轮的内容
if len(summary_text) < 100:
    summary_text += f" The analysis integrated multiple data sources..."
```
...
**建议**: 
- 添加验证逻辑，确保 coordinator_summary 被包含在返回结果中
- 或者在返回结果中显式添加 `coordinator_summary` 字段
...
| **Coordinator** | `discussion.coordinator_summary` | ⚠️ 缺失 | 应该>100字符 | 需要验证传递 |
...
**下一步**:
1. ✅ Coordinator summary 生成已修复（至少100字符）
2. ⚠️ 需要验证 coordinator_summary 是否被正确传递到返回结果
3. ✅ 其他 agent 的 summary 都正常
