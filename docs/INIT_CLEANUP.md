...
以下内容会被保留：
- `memory/` 目录结构 - 文件夹本身会被保留
- `.gitkeep` 文件 - Git占位文件（应该保留）
- **备份文件** - 默认保留（但会清理7天前的旧备份）

### 清理 memory/ 目录

**重要变更**：初始化时会清理 `memory/` 目录下的**所有文件**（但保留文件夹结构）：
- 删除 `memory/daily/` 下的所有 `.json` 文件
- 删除 `memory/weekly/` 下的所有 `.jsonl` 文件
- 删除 `memory/monthly/` 下的所有文件
- 删除 `memory/index/` 下的所有文件
- **保留**所有文件夹结构（`daily/`, `weekly/`, `monthly/`, `index/` 文件夹本身）

这样确保初始化后 `memory/` 目录是干净的，Agent 会从零开始学习。
