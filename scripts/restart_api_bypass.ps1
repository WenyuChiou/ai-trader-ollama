# PowerShell Script: Restart API Server (Bypass Execution Policy)
# This script bypasses execution policy restrictions
# Usage: powershell -ExecutionPolicy Bypass -File .\restart_api_bypass.ps1
...
if ($portConnections) {
    ...
    foreach ($pid in $processIds) {
        ...
        if ($process) {
            Write-Host "  Found process: PID $pid ($($process.ProcessName))" -ForegroundColor Yellow
