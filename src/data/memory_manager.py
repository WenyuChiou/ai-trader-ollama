...
class MemoryManager:
    ...
    def save_daily_memory(
        ...
        *,
        ...
        try:
            ...
            safe_print(f"[MEMORY] Saved daily memory for {date}")
        except Exception as e:
            safe_print(f"[MEMORY ERROR] Failed to save memory file: {e}")
            import traceback
            try:
                traceback.print_exc()
        ...
        try:
            ...
            safe_print(f"[MEMORY ERROR] Failed to update index: {e}")
            import traceback
            try:
                traceback.print_exc()
            except (ValueError, OSError):
        ...
        if compress_old:
            try:
                ...
                safe_print(f"[MEMORY ERROR] Failed to compress old memories: {e}")
                # 不抛出异常，不影响主流程
    ...
    def _update_index(self, date: str, memory: Dict[str, Any]) -> None:
        ...
        try:
            ...
            with temp_index_file.open("w", encoding="utf-8") as f:
            ...
            safe_print(f"[MEMORY ERROR] Failed to save index: {e}")
            import traceback
            try:
                traceback.print_exc()
            except (ValueError, OSError):
        ...
        for memory_file in self.daily_dir.glob("*.json"):
            if memory_file.stem < cutoff_date:
                try:
                    ...
                    # 创建压缩版本（只保留摘要）
                    compressed = self._create_compressed_memory(memory)
                    ...
                    safe_print(f"[MEMORY] Compressed {memory_file.stem} to weekly archive")
                except Exception as e:
                    safe_print(f"[MEMORY WARN] Failed to compress {memory_file.stem}: {e}")
    ...
    def load_daily_memory(self, date: str) -> Optional[Dict[str, Any]]:
        ...
        if daily_file.exists():
            try:
                ...
                safe_print(f"[MEMORY ERROR] Failed to load {date}: {e}")
