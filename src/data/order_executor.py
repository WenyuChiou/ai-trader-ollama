...
def get_current_or_open_price(symbol: str, target_date: str) -> Optional[float]:
    ...
    **重要限制**：
    ...
    - 如果遇到假期或退市股票（YFPricesMissingError），静默忽略并返回 None
    ...
    try:
        ...
        # 如果是今天，优先使用实时价格（regularMarketPrice）
        ...
        if hist is not None and not hist.empty:
            ...
            if "Open" in hist.columns:
                ...
                if isinstance(open_price, pd.Series):
                    open_price = open_price.iloc[0] if len(open_price) > 0 else None
            ...
            if "Close" in hist.columns:
                ...
                if isinstance(close_price, pd.Series):
                    close_price = close_price.iloc[0] if len(close_price) > 0 else None
                if close_price is not None and not pd.isna(close_price):
                    return float(close_price)
        
        # 如果历史数据不可用，尝试使用最新价格（实时或最近的收盘价）
        try:
            info = ticker.info
            if "regularMarketPrice" in info and info["regularMarketPrice"]:
                return float(info["regularMarketPrice"])
        ...
        if "YFPricesMissingError" in error_str or "possibly delisted" in error_str or "no price data found" in error_str:
            ...
            return None
        # 其他错误也静默处理，但可以选择记录
        # safe_print(f"[ORDER EXECUTOR] Failed to get price for {symbol}: {e}")
        return None
    ...
    for order in orders:
        ...
        if not execution_check["can_execute"]:
            ...
            if status == "REJECTED":
                ...
                # 等待成交（不记录为错误，但也不执行）
                safe_print(f"[ORDER PENDING] {symbol}: {reason}")
