...
def main():
    ...
    convo = run_analyst_discussion(
        ...
        rounds=rounds,
        auto_tools=True,
        tool_budget=3,  # 允許它再補 vix_term/news_scan 等
        preferred_domains=domains,
    )
    ...
    # ↓↓↓ 改這行：支援 'type' 或 'action'，避免出現 ['', '', '']
    actions = [a.get("type") or a.get("action") or "" for a in convo.get("actions", [])]
    print("\n[ACTIONS]", actions)
